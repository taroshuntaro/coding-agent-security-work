...
| データ系キー | `enableArtifact`（`false` で無効化。どのスコープからも再有効化不可。v2.1.242 以降）／ `disableArtifact` は **deprecated**（`true` は同等扱い）／ `disableRemoteControl` / `disableClaudeAiConnectors` / `autoMemoryEnabled` / `cleanupPeriodDays`（既定 30 日） | ✅ | 2026-09-21 | [settings-reference](https://code.claude.com/docs/en/settings-reference) |
